...
let generate infilename outfilename =
  ...
  *)
  ...
  let flat_code_fragments = Flattener.flatten_program code in
  let c_file = CodeGenerator.c_of_flat_program flat_code_fragments in
...
let main () =
  ...
  then print_endline
    ...
    ^ "an input filename and an output filename")
